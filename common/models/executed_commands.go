...
// LogExecutedCommandToDatabase logs an executed command to the database
func LogExecutedCommandToDatabase(ctx context.Context, exec boil.ContextExecutor, cmd *ExecutedCommand) error {
	return cmd.Insert(ctx, exec, boil.Infer())
}

// LogExecutedCommandToRedis logs an executed command to Redis
func LogExecutedCommandToRedis(ctx context.Context, pool *radix.Pool, cmd *ExecutedCommand) error {
	key := fmt.Sprintf("executed_command:%d", cmd.ID)
	err := pool.Do(radix.FlatCmd(nil, "HMSET", key, "id", cmd.ID, "created_at", cmd.CreatedAt, "updated_at", cmd.UpdatedAt, "user_id", cmd.UserID, "channel_id", cmd.ChannelID, "guild_id", cmd.GuildID, "command", cmd.Command, "raw_command", cmd.RawCommand, "error", cmd.Error, "time_stamp", cmd.TimeStamp, "response_time", cmd.ResponseTime))
	if err != nil {
		return errors.Wrap(err, "models: unable to log executed command to redis")
	}
	return nil
}
